...
Existing animated pill indicator and mobile hamburger menu preserved.
...
3. **Numbered Features [01]-[04]** — JetBrains Mono numbering. Features:
...
4. **Agent Ecosystem Flow Diagram** — Full data flow visualization. Left column: input sources (channels + meeting tools). Center: 5 agents with gradient borders. Right column: outputs (Lead, Proposal PDF, Customer Report, Meeting Summary, Knowledge Base). Animated SVG connectors between columns.
...
- **Sections numbered**: [01] [02] [03] with JetBrains Mono
...
- Existing dictionary structure preserved

### New Components
- `src/components/sections/flow-diagram.tsx` — Workflow visualization with SVG connectors
...
- `src/components/sections/data-model-viz.tsx` — Source → Hub → Output flow visualization
- `src/components/sections/scroll-pinned-features.tsx` — Sticky scroll-driven section
...
- **CSS @property**: `--gradient-angle` for animatable conic-gradient rotation
- **IntersectionObserver**: Scroll-pinned section content triggers, navbar active state
...
- `src/components/sections/navbar.tsx` — Update links for multi-page navigation
